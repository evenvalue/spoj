...
public:
  ...
  int operate(int x, int y, const F &op, bool ignore_lca) {
    while (root[x] != root[y]) {
    ...
    if (pos[x] + ignore_lca <= pos[y]) op(pos[x] + ignore_lca, pos[y]);
    return x;
  }
};
...
inline void solution() {
  ...
  auto fix = [&](const int i, const int val) {
    hld.operate(edges[i].y, edges[i].y, [&](const int l, const int r){
      assert(l == r);
      st.update(l, val);
    }, false);
  };
